  ...
  // ✅ ADICIONE AQUI
  const toastPersistente = sessionStorage.getItem("toast");

  if (toastPersistente) {
    const { message, type } = JSON.parse(toastPersistente);
...
window.addEventListener("pageshow", (event) => {
  ...
  if (formCadastro) {
    formCadastro.addEventListener("submit", async (e) => {
      ...
      const nome = formCadastro.querySelector(
        'input[placeholder="Nome completo"]',
      ).value;
      const email = formCadastro.querySelector(
      ...
      try {
        ...
        showToast(
          "Cadastro enviado com sucesso! Aguarde aprovação.",
          "success",
        );
...
// === ROTEAMENTO CENTRAL ===
function redirecionarPorPerfil(payload) {
  ...
  const rotasPorSetor = {
    LOGISTICA: "/dashboard-log.html",
    FINANCEIRO: "/dashboard-fin.html",
    RH: "/dashboard-rh.html",
    COMERCIAL: "/dashboard.html",
  };

  window.location.href = rotasPorSetor[payload.setor] || "/dashboard.html";
}
